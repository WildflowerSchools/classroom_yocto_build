...
SRC_URI = "https://files.pythonhosted.org/packages/source/s/${SRCNAME}/${SRCNAME}-${PV}.tar.gz \
           ...
           file://salt-api.service \
           file://salt-master.service \
           file://master \
           file://salt-syndic.service \
           file://cloud \
           file://roster \
...
do_install_append() {
        ...
        install -m 0644 ${WORKDIR}/salt-minion.service ${D}${systemd_unitdir}/system
        install -m 0644 ${WORKDIR}/salt-api.service    ${D}${systemd_unitdir}/system
        install -m 0644 ${WORKDIR}/salt-master.service ${D}${systemd_unitdir}/system
        install -m 0644 ${WORKDIR}/salt-syndic.service ${D}${systemd_unitdir}/system
        ...
        install -d ${D}${sysconfdir}/${PN}/cloud.conf.d \
                   ${D}${sysconfdir}/${PN}/cloud.profiles.d \
                   ${D}${sysconfdir}/${PN}/cloud.providers.d
...
SYSTEMD_PACKAGES = "${PN}-minion ${PN}-api ${PN}-master ${PN}-syndic"
...
RDEPENDS_${PN}-minion = "python3-modules (>=2.6), ${PN}-common (= ${EXTENDPKGV}) python3-msgpack python3-pyyaml"
...
RDEPENDS_${PN}-common = "python3-core (>= 3.4.0) python3-core (< 3.7) python3-dateutil python3-jinja2 python3-pyyaml python3-requests (>= 1.0.0) python3-tornado (>= 4.2.1)"
RRECOMMENDS_${PN}-common = "lsb python3-misc"
...
RDEPENDS_${PN}-ssh = "python3-core (>= 2.6) ${PN}-common (= ${EXTENDPKGV}) python3-msgpack"
...
RDEPENDS_${PN}-api = "python3-core (>= 2.6) ${PN}-master"
RSUGGESTS_${PN}-api = "python3-cherrypy"
FILES_${PN}-api = "${bindir}/${PN}-api ${CONFFILES_${PN}-api}"
SYSTEMD_SERVICE_${PN}-api = "${PN}-api.service"
...
RDEPENDS_${PN}-master = "python3-core (>= 2.6) ${PN}-common (= ${EXTENDPKGV}) python3-msgpack"
...
CONFFILES_${PN}-master="${sysconfdir}/${PN}/master"
...
SYSTEMD_SERVICE_${PN}-master = "${PN}-master.service"
...
RDEPENDS_${PN}-syndic = "python3-core (>= 2.6) ${PN}-master (= ${EXTENDPKGV})"
FILES_${PN}-syndic = "${bindir}/${PN}-syndic ${CONFFILES_${PN}-syndic}"
SYSTEMD_SERVICE_${PN}-syndic = "${PN}-syndic.service"
...
RDEPENDS_${PN}-cloud = "python3-core (>= 2.6) ${PN}-common (= ${EXTENDPKGV})"
...
RDEPENDS_${PN}-tests = "${PN}-common python3-salttesting python3-tests python3-image bash"
